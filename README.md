...
user one shared shell, one editable virtual filesystem, and one small agent loop
instead of a separate workflow UI.

The extension seeds its editable project files into IndexedDB, then runs
`/src/agent.js` through the DietSurf kernel. After first load, the virtual files
inside the extension are the live source of truth; the packaged files are reset
material and defaults.

## What It Does

- Opens as a Chrome side panel with a terminal-like prompt.
- Runs direct shell commands such as `ls`, `cat`, `grep`, `node`, `sed`, and
  heredoc writes.
- Treats non-shell input as a goal and routes it through `/src/agent.js`.
...
`/etc/llm.json` file from the side-panel shell. For local Node mode, `.env` can
provide `LILAC_API_KEY`.
